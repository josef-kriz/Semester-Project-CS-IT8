...
# output = sys.argv[4]
output = 'data/incidents-normal-operation.csv'

dest_file = open(output, 'w')

print('fetching normal machines')

cursor.execute("SELECT DISTINCT anlaeg_id "
               "FROM anlaegshaendelser "
               "WHERE anlaeg_id NOT IN ( "
                    "SELECT DISTINCT anlaeg_id "
                    "FROM anlaegshaendelser "
                    "WHERE haendelse = 112"
               ");")
...
print(str(len(ids)) + " normal operating machines found.")

ids = ','.join(ids)
ids = '(' + ids + ')'

misfires = []
cursor.execute("SELECT DISTINCT DATE_ADD('2018-05-01 13:00:00', INTERVAL 1 SECOND), anlaeg_id FROM anlaegshaendelser WHERE anlaeg_id IN " + ids)
misfires.extend(cursor.fetchall())
print("fake incidents fetched")
cursor.execute("SELECT DISTINCT DATE_ADD('2017-12-01 13:00:00', INTERVAL 1 SECOND), anlaeg_id FROM anlaegshaendelser WHERE anlaeg_id IN " + ids)
misfires.extend(cursor.fetchall())
print("fake incidents fetched")
cursor.execute("SELECT DISTINCT DATE_ADD('2017-03-01 13:00:00', INTERVAL 1 SECOND), anlaeg_id FROM anlaegshaendelser WHERE anlaeg_id IN " + ids)
misfires.extend(cursor.fetchall())
print("fake incidents fetched")
cursor.execute("SELECT DISTINCT DATE_ADD('2016-09-01 13:00:00', INTERVAL 1 SECOND), anlaeg_id FROM anlaegshaendelser WHERE anlaeg_id IN " + ids)
misfires.extend(cursor.fetchall())
print("fake incidents fetched")
cursor.execute("SELECT DISTINCT DATE_ADD('2015-08-01 13:00:00', INTERVAL 1 SECOND), anlaeg_id FROM anlaegshaendelser WHERE anlaeg_id IN " + ids)
misfires.extend(cursor.fetchall())
print("fake incidents fetched")

# go through all cluster groups and collect the data into a dictionary
# cursor.execute('SELECT opkdato, anlaeg_id FROM anlaegshaendelser WHERE haendelse = 112 AND misfire_shutdown = 0 ORDER BY opkdato')
