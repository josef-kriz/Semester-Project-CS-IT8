...
parsers = [
    ...
    [unsigned_parser, 3, 1, ['bp_flowmasterpsuvoltage', 'cv_psu_voltage', 'dwell_time', 'ecu_psu_voltage', 'ignition_angel', 'ignitor_current', 'injector_current', 'varmefordeler_transport']],
    [unsigned_parser, 3, 2, ['nb_current']],
    [unsigned_parser, 4, 2, ['ca_netfrekvens', 'fl', 'fm']],
    [temp_parser, 6, 2, ['varmefordeler_printtemparatur','st','mv','gassikker_printtemp','ad','ae','ag','ah','an','ao','bm_flowmastersrctemp','bn_flowmasterbypasstemp','bo_flowmasterpcbtemp','ckAuxTempTrackerSensor1','clAuxTempTrackerSensor2','cmAuxTempTrackerSensor3','cnAuxTempTrackerSensor4','DXHPC1HPVTemp','DYHPC1HPKTemp','ecu_vandtemp','EGHPC2HPVTemp','EIHPC2HPKTemp','EQHPC3HPVTemp','ERHPC3HPKTemp','ecu_pcb_temp','FAHPC4HPVTemp','FCHPC4HPKTemp','flow_control_fremloebs','FKHPC5HPVTemp','FNHPC5HPKTemp','flow_control_setpoint','flow_control_retur','flowcontrolusersetpoint','FYHPC6HPVTemp','FZHPC6HPKTemp','GGPMH1HpvTemp','GHPMH1HpkTemp','GPPMH2HpvTemp','GQPMH2HpkTemp','gt','hb','HDPMH3HpvTemp','HFPMH3HpkTemp','HNPMH4HpvTemp','HOPMH4HpkTemp','ht','HYPMH5HpvTemp','HZPMH5HpkTemp','IKPMH6HpvTemp','ILPMH6HpkTemp','ISSmartStartBoardTemp','IXPMHStorageTemp','lk','lw','mk','ps','vr']],
    [signed_parser, 6, 0, ['ai', 'aj', 'ak', 'am']],
    [unsigned_parser, 5, 0, ['aim_power', 'av', 'misfire_1', 'lgvgp', 'requested_power']],
    [unsigned_parser, 4, 0, ['actual_map_pressure', 'start_map_pressure', 'map_offset', 'lambda_setpoint', 'lambda_voltage']],
